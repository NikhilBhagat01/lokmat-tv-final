...
export async function generateMetadata({ params }) {
  ...
  const meta = {
    title: `${categoryName} Playlist - Page ${number} Lokmat TV Videos | Lokmat.com`,
...
const page = async ({ params }) => {
  ...
  const breadcrumbJsonld = getBreadcrumbListJsonld([
    ...
    {
      name: `${data?.playlistName} Playlist - Page ${number}`,
      url: `${GLOBAL_CONFIG.SITE_PATH}/playlist/${slug}/${videoId}/page/${number}`,
    },
  ]);
  ...
  return (
    <>
      <JsonLdWebPage
        url={`${GLOBAL_CONFIG.SITE_PATH}/playlist/${slug}/${videoId}`}
        name={`${data?.playlistName} Playlist - Page ${number}`}
        description={`${data?.playlistName} Playlist - Page ${number}`}
      />
      <script
