...
export async function generateMetadata({ params }) {
  ...
  const meta = categoryMetadataMap[categoryName] || {
    title: `${categoryName} - Page ${number} Lokmat TV Videos`,
...
const page = async ({ params }) => {
  ...
  const breadcrumbJsonld = getBreadcrumbListJsonld([
    ...
    {
      name: `${data?.categoryName} - Page ${number}`,
      url: `${GLOBAL_CONFIG.SITE_PATH}/videos/${data?.slug}/page/${number}`,
    },
  ]);
  ...
  return (
    <>
      <JsonLdWebPage
        url={`${GLOBAL_CONFIG.SITE_PATH}/videos/${slug}/page/${number}`}
        name={`${data?.categoryName} - Page ${number}`}
        description={`${data?.categoryName} - Page ${number}`}
      />
      <script
